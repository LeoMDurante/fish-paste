...
save(wd_ind_fish_MHI, file="TMPwd_ind_fish_MHI.Rdata")  #Save clean working data
...
survey_table<-Aggregate_InputTable(wd_ind_fish_MHI, SURVEY_INFO)

OTHER_BENTHIC<-c("CLAM", "CORALLIMORPH", "ZOANTHID", "TUNICATE", "SPONGE", "TA", "CYANO", "OTHER", "SOFT_CORAL")
wd_ind_fish_MHI$OTHER_BENTHIC<-rowSums(wd_ind_fish_MHI[,OTHER_BENTHIC],na.rm=T)
...
survey_est_benthos<-Calc_Site_nSurveysArea(wd_ind_fish_MHI, UNIQUE_SURVEY, UNIQUE_REP, UNIQUE_COUNT, SURVEY_SITE_DATA)   #Calc_Site_nSurveysArea deals better with situations where one REP has benthic data and other doesnt. 
surveys<-merge(survey_table, survey_est_benthos, by=UNIQUE_SURVEY)
save(surveys, file="TMPsurveys_MHI.Rdata")
...
species_table<-Aggregate_InputTable(wd_ind_fish_MHI, FISH_SPECIES_FIELDS)
save(species_table, file="TMPspecies_MHI.Rdata")

# GENERATE SUMMARY METRICS --------------------------------------------------
r1<-Calc_Site_Bio(wd_ind_fish_MHI, "TROPHIC_MONREP"); trophic.cols<-names(r1[3:dim(r1)[2]])
...
save(wsd, file="TMPwsd_MHI.Rdata")
save(data.cols, file="TMPdata.cols_MHI.Rdata")


## --------------------------- REMOVED FROM HERE BY LEO ----------------------- ##
